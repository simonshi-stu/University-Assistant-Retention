...
## Course Explorer normalized tables

The cache-first Course Explorer extension uses the primary terms `2021`, `2022`,
and `2023` (spring, summer, and fall when requested). It never infers section
facts from the GPA source. The normalized CSV tables are written under
`data/processed/tables/`:

| Table | Grain and key | Important fields |
| --- | --- | --- |
| `courses.csv` | one term + subject + catalog number; `course_id` | title, description, credit range/text |
| `sections.csv` | one term + CRN; `section_id` | section number, Part of Term, date range, credit |
| `meetings.csv` | one row per section meeting; `section_id + meeting_index` | meeting type, weekday, start/end, building, room, date range, ARRANGED/TBA flags |
| `instructors.csv` | one section instructor; section + instructor index/name | name, email, role |
| `gened.csv` | one section + GenEd attribute | code and description |
| `prerequisites.csv` | one course prerequisite text | prerequisite text |
| `conflict_edges.csv` | one section pair + shared weekday + overlapping meeting pair | date intersection, both intervals, overlap minutes |

`acquisition_records.csv` records one subject/course resource attempt with URL,
status, HTTP status when available, SHA-256, byte count, cache path, timestamp,
source/normalized row counts and missing critical fields. `quality_report.csv`
aggregates the same evidence per term and subject and sets `verified=true` only
when parsed rows exist and critical fields are complete. A blocked HTTP response or
cache miss is evidence of unavailable data, not a zero-row source.

The conflict engine requires different sections in the same term, overlapping
date ranges, at least one shared weekday, and a strictly positive interval
overlap. `ARRANGED` and `TBA` meetings are retained with flags but are excluded.
These edges are timetable-overlap planning evidence, not observed student
registration conflicts: the public inputs do not contain student IDs,
registration events, or section-level enrollment snapshots.
